...
-- ── 4. PUSH SUBSCRIPTIONS (web push channel) ────────────────
-- One row per browser the user turned notifications on in.
-- Client inserts/deletes its own rows; the alert engine (service
-- role) reads all rows and prunes dead endpoints (404/410).
