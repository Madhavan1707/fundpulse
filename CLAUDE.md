...
- **Alert engine** runs 3× nightly (21:35 / 22:35 / 23:35 IST) via GitHub Actions. Retries transient NAV failures, evaluates the newest NAV within 4 days, dedups via `alert_log` on `nav_date`, exits green when there's nothing fresh.
  **Do not reintroduce a `navDate === today` freshness gate.** GitHub delayed this cron by +36m to +8h34m over the week of 2026-08-24; every run that landed after IST midnight then skipped real NAVs as "stale" and reported success having sent nothing, and no later run could recover that date. `alert_log`'s unique `(user_id, fund_id, alert_type, nav_date)` is what prevents duplicate sends.
...
|---|---|---|
| mfapi.in | Live NAV data + fund search | Free, no key, single droplet with no CDN. Engine retries 4× (5s/20s/60s backoff) with 15s timeout |
| NewsData.io | News feed | Free tier; fetched every 2h by GitHub Actions |
| GitHub Actions cron | Alert engine, 3× nightly at 21:35 / 22:35 / 23:35 IST (`5 16,17,18 * * *` UTC) | GitHub delays scheduled runs by hours, so one slot is not enough. Accepts the newest NAV within 4 days; `alert_log` dedups on `nav_date` so the extra runs are no-ops |
